...
useEffect(() => {
  const fetchPouches = async () => {
    ...
    try {
      ...
      if (result.success && result.data?.pouches) {
        ...
        // let dept = selectedDepartment.toLowerCase();

        // setTempOrder(result.data[dept]?.Order_Id__c);

        // setTempProduct(result.data[dept]?.Product__c);        
        // setTempQuantity(result.data[dept]?.Quantity__c);

          let dept = selectedDepartment.toLowerCase();

          // Normalize API shape (object or array)
          const deptData = Array.isArray(result.data[dept])
            ? result.data[dept][0]
            : result.data[dept];

          setTempOrder(deptData?.Order_Id__c);
          setTempProduct(deptData?.Product__c);
          setTempQuantity(deptData?.Quantity__c);
  ...
  useEffect(() => {
    if (selectedRecord) {
      ...
      const recordData = departmentRecords.find(record => record.Name === selectedRecord);
  ...
  try {
    ...
    const requestData = {
        ...
        orderId: tempOrder || null,
        product: tempProduct || null,
        quantity: tempQuantity || null,
        pouches: pouchData,
         status: "Pending"
      };

      console.log("temp : ", tempOrder, tempProduct, tempQuantity);
    ...
    if (result.success) {
      ...
      alert("Dull record created successfully");
  ...
  return (
    <div className="p-6">
      <div className="max-w-6xl mx-auto bg-white rounded-lg shadow-sm">
        <div className="p-6">
          <h2 className="text-xl font-semibold mb-6">Create Dull from Department</h2>
          
          <form onSubmit={handleSubmit} className="space-y-6">
            {/* Department Selection - Modified to only show Dull */}
            <div>
            ...
            <Button
              ...
              className="w-full"
            >
              {loading ? 'Creating...' : 'Create Dull Record'}
            </Button>
          </form>
        </div>
      </div>
