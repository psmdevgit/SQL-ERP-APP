...
useEffect(() => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    for (const pouch of data.pouches) {
      ...
      alert('Submitting correction details...');
      console.log('[correctionReceived] Submitting data:', formData);
      const response = await fetch(
        `${apiBaseUrl}/api/correction/update/${idParts.prefix}/${idParts.date}/${idParts.month}/${idParts.year}/${idParts.number}/${idParts.subnumber}`,
        {
          method: 'POST',
          headers: {
      ...
      alert('Failed to update Correction details');
      toast.error(error.message || 'Failed to update Correction details');
    } finally {
