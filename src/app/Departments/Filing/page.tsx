...
//       if (result.success) {
//         // Filter on correct field name
//         const receivedRecords = result.data.filter(
//           (record: DepartmentRecord) =>
//             record.status__c === "Finished" ||
//             record.status__c === "Completed"
//         );

//         console.log("Filtered Records:", receivedRecords);
...
const handleSubmit = async (e: React.FormEvent) => {
  ...
  try {
    ...
    if (pouchData.length === 0) {
      console.warn("⚠️ pouchData is empty! Check pouch selection or ID mapping.");
    }

    // Get first pouch for filing summary
  ...
  return (
    <div className="p-6">
      <div className="max-w-6xl mx-auto bg-white rounded-lg shadow-sm">
        <div className="p-6">
          <h2 className="text-xl font-semibold mb-6">Create Filing from Department</h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <Button
              ...
              className="w-full"
            >
              {loading ? 'Creating...' : 'Create Filing Record'}
            </Button>
          </form>
        </div>
      </div>
