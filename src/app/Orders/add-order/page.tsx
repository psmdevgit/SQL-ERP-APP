...
const handleModelSelect = (modelId) => {
  ...
  const fetchPartyLedgers = async () => {
    try {
      ...
      const result = await response.json();
      console.log("party ledger", result);
      if (response.ok && result.success) {
        setPartyLedgers(result.data.map((ledger: any) => ledger.Party_Code_c));
      }
    } catch (error) {
  ...
  const { category, wtRange, size, quantity, itemRemark } = formData;
  ...
  // If all fields valid, create new item
  const newItem = {
    category,
    weightRange: wtRange,
    size,
...
<td className="px-4 py-2">
  <input
    ...
    onChange={(e) => {
      console.log("grs wt : ", item.grossWeight);
      const updated = [...orderSelectedItems];
