 ...
 const fetchItems = async (category: string) => {
  try {
    ...
    if (result.success) {
      ...
      console.log(result.message || "No items found for this category.");
      setItems([]);
    }
  } catch (error) {
...
const handleItemSelect = async (itemName) => {
  try {
    ...
    if (selectedItem) {
      ...
      // Handle image URL
      if (imageUrl) {
        const downloadUrl = `${apiBaseUrl}${imageUrl}`;
...
const handleAdd = () => {
  if (!formData.category || !formData.item || !formData.quantity || !formData.purity) {
     ...
     try {
       ...
       if (result.success) {
        ...
        //  });
         ...
         console.log("Models and PDFs saved successfully");
         // Preview PDFs
        ...
        //  window.open(URL.createObjectURL(imagesPdfBlob), '_blank');
         ...
         console.log("Failed to save models and PDFs");
