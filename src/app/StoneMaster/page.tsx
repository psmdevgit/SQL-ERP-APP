...
const fetchStonemaster = async () => {
  try {
    ...
    if (data.success) {
      // console.log("stone master",data.result.records);
      const records = data.result.records; // ✅ use data.data (not data.result.records)
      setStoneData(records);
// console.log("stone records : ", records);

      // Extract unique types
      const uniqueTypes = [
        ...new Set(records.map((item) => item.type_c)) // ✅ match your SQL field
      ].map((typeName, index) => ({
        id: index + 1,
        name: typeName
      }));
...
useEffect(() => {
  ...
  if (selectedType && selectedType !== customType) {
    ...
    const filtered = stoneData.filter(item => item.type_c === selectedType);
    ...
    if (filtered.length > 0) {
      ...
      setColor(lastRecord.colour_c);
      setShape(lastRecord.Shape_c || '');
      setSize(lastRecord.size_c || '');
    } else {
      setColor("");
      setShape("");
      setSize("");
