...
const SettingSummary: React.FC = () => {
  const [settingData, setSettingngData] = useState<ISetting[]>([]);
  ...
  //   const totalFilings = settingData.length;
  //   const totalIssuedWeight = settingData.reduce(
  //     (sum, item) => sum + (Number(item.issuedWeight) || 0),
  //     0
  //   );
  //   const totalReceivedWeight = settingData.reduce(
  //     (sum, item) => sum + (Number(item.receivedWeight) || 0),
  //     0
  //   );
  //   const totalFilingLoss = settingData.reduce(
  //     (sum, item) => sum + (Number(item.settingLoss) || 0),
  //     0
  //   );
  //   const totalProcessingWeight = settingData.reduce((sum, item) => {
  ...
  // Today's data only
  const todayData = settingData.filter(
    (item) =>
  ...
  return [
    ...
    },
    {
      ...
      title: "Setting Issued",
      value: totalFilings.toString(),
      description: "Today’s Setting jobs",
      percentageChange: "",
      isIncrease: true,
    },
    {
  ...
  <>
   {/* ✅ Row 1 - Processing Weight */}
    ...
    {/* ✅ Row 3 - Other 4 cards */}
 {/* Row 3 - Other 4 cards */}
<div className="w-full max-w-7xl grid grid-cols-1 gap-2 mt-2">
