...
const GrindingSummary: React.FC = () => {
  const [grindingData, setGrindingData] = useState<IFiling[]>([]);
  ...
  // Today's data only
  const todayData = grindingData.filter(
    (item) =>
      item.issuedDate && item.issuedDate.split("T")[0] === today
  );

  // ---- All data (for processing weight only) ----
  const totalProcessingWeight = grindingData.reduce((sum, item) => {
  ...
  return [
    ...
    },
    {
      ...
      description: "Today’s Grinding jobs",
      percentageChange: "",
      isIncrease: true,
    },
    {
