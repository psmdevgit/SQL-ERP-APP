...
const PlatingSummary: React.FC = () => {
  ...
  // Today's data only
  const todayData = platingData.filter(
    (item) =>
      item.Issued_Date__c && item.Issued_Date__c.split("T")[0] === today
  );

  // ---- All data (for processing weight only) ----
  const totalProcessingWeight = platingData.reduce((sum, item) => {
    const received = Number(item.Received_Weight__c || 0);
    return received ? sum : sum + Number(item.Issued_Weight__c || 0);
  }, 0);
  ...
  const totalReceivedWeight = todayData.reduce(
    (sum, item) => sum + (Number(item.Received_Weight__c) || 0),
    0
  );
  ...
  return [
    ...
    },
    {
      ...
      description: "Today’s Plating jobs",
      percentageChange: "",
      isIncrease: true,
    },
    {
