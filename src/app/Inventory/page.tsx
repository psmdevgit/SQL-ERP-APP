...
const scrapItems = ["casting scrap", "scrap", "Assesmbly Finding"];
...
// item !== "casting scrap"  && item !== "scrap" 
 const item = formData.itemName.toLowerCase();

if (item !== "finding" && item !== "assesmbly finding"  ) {
              ...
              >
                ...
                <SelectContent className="bg-white border border-gray-200">
                  {partyLedgers.map((ledger) => (
                    <SelectItem key={ledger.Id} value={ledger.Party_Code_c}>
                      {ledger.Party_Code_c}
                    </SelectItem>
                  ))}
                </SelectContent>
...
<div className="space-y-2">
  ...
  <Select
    ...
    disabled={!formData.partyLedger}  // ⬅️ Disable if no Party Ledger selected
  >
    <SelectTrigger
      className={`w-full border border-gray-200 ${
        !formData.partyLedger ? "bg-gray-100 cursor-not-allowed opacity-70" : "bg-white"
      }`}
    >
