...
const OrderDetailsPage = () => {
  ...
  const [processingWeights, setProcessingWeights] = useState<{ process: string; ReceivedWt: number; IssuedWt: number; ProcessWt: number; }[]>([]);
  ...
  useEffect(() => {
    const fetchDetails = async () => {
      ...
      if (result.success) {
                ...
                // loop through all regular models and fetch their image
                for (const model of result.data.regularModels) {
                  try {
                    const imgRes = await fetch(`${apiBaseUrl}/api/previewImage?modelName=${encodeURIComponent(model.name)}`);
                    const imgData = await imgRes.json();

                    console.log(imgData);

                    if (imgData.success && imgData.data.length > 0) {
                      map[model.name] = imgData.data[0].image;
                      console.log(map[model.name] = imgData.data[0].image)
                    }
                  } catch (err) {
                    console.error("Error fetching image for model:", model.name, err);
                  }
                }

                setModelImages(map);

                  // fetch processing weights
              const pwRes = await fetch(`${apiBaseUrl}/api/order-processing-weights?orderId=${orderId}`);
              const pwData = await pwRes.json();
              if (pwData.success) {
                setProcessingWeights(pwData.data);
                console.log("process wt : ", processingWeights)
              }
    ...
    >
      ...
      >
        ...
        {regularModels.length > 0 && (
          <div className="bg-white shadow rounded-lg mb-6 w-full">
            <div className="px-6 py-4 border-b border-gray-200">
              <h3 className="text-lg font-semibold">Model Images</h3>
            </div>
...
{processingWeights.length > 0 && (
  <div className="bg-white shadow rounded-lg mb-6 w-full max-w-md ">
    <div className="px-4 py-3 border-b border-gray-200">
      <h3 className="text-md font-semibold text-gray-800">
        Processing Weight of this Order
      </h3>
    </div>
    <div className="overflow-x-auto">
      <table className="w-full text-sm" style={{ minWidth: "300px" }}>
        <thead style={{ backgroundColor: "#1A7A75" }}>
          <tr>
        ...
        <tbody className="bg-white divide-y divide-gray-200">
          {processingWeights.map((item, idx) => (
