...
const InventoryItemSummary: React.FC = () => {
  ...
  const handleOpening = async () => {
    setLoading(true);
    const res = await fetch(`${API_URL}/api/opening`, { method: "GET" });
  ...
  const handleClosing = async () => {
    setLoading(true);
    const res = await fetch(`${API_URL}/api/closing`, { method: "GET" });
    const data = await res.json();
    alert(data.message);
    setLoading(false);
  };
...
<div className="max-w-screen-md mx-auto mt-5 p-6 bg-white shadow rounded-lg">
 <div className="flex gap-4 p-5">
      <Button onClick={handleOpening} disabled={loading}>
        Opening Stock
      </Button>

      <Button onClick={handleClosing} disabled={loading} className="bg-success">
        Closing Stock
      </Button>
    </div>
</div>
