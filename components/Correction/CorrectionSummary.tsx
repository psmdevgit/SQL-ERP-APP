...
import { fetchCorrectionData } from "@/data/crm/correction-data";
import { IFiling, ICorrection } from "@/interface/table.interface";
import DatePicker from "react-datepicker";
import "react-datepicker/dist/react-datepicker.css";

    const CorrectionSummary: React.FC = () => {
  const [CorrectionData, setCorrectionData] = useState<ICorrection[]>([]);
  const [filteredData, setFilteredData] = useState<ICorrection[]>([]);
  ...
  useEffect(() => {
    const getData = async () => {
      try {
        setLoading(true);
        const data = await fetchCorrectionData();
  ...
  useEffect(() => {
    filterDataByDateRange(CorrectionData, dateRange);
  }, [dateRange, customStartDate, customEndDate, CorrectionData]);

  // Function to filter data by date range
  const filterDataByDateRange = (data: ICorrection[], range: string) => {
    if (!data || !data.length) {
  ...
  const filterByDateRange = (data: ICorrection[], start: Date, end: Date) => {
  ...
  const calculateSummary = () => {
    const totalCorrection = filteredData.length;
    ...
    const totalCorrectionLoss = filteredData.reduce(
     ...
     const totalCorrectionDust = filteredData.reduce(
          (sum, item) => sum + Number(item.correctionDust || 0),
      0
    );
  ...
  const totalProcessingWeight = filteredData.reduce((sum, item) => {
    ...
    const CorrectionLossPercentage = totalIssuedWeight
      ? ((totalCorrectionLoss / totalIssuedWeight) * 100).toFixed(2)
      : "0";
    
       const CorrectionDustPercentage = totalIssuedWeight
      ? ((totalCorrectionDust / totalIssuedWeight) * 100).toFixed(2)
      : "0";
    ...
    return [
      {
        ...
        description: "Total Correction jobs",
        percentageChange: "",
        isIncrease: true,
      },
    {
      ...
      },  
      {
        ...
        value: totalCorrectionDust.toFixed(2) + " g",
        description: CorrectionDustPercentage + "% of issued",
        percentageChange: CorrectionDustPercentage,
        isIncrease: false
      },
      {
        iconClass: "fa-light fa-arrow-trend-down",
        title: "Correction Loss",
        value: totalCorrectionLoss.toFixed(2) + " g",
        description: CorrectionLossPercentage + "% of issued",
        percentageChange: CorrectionLossPercentage,
        isIncrease: false
      },
    ];
  };
  ...
  const handleApplyCustomRange = () => {
    if (customStartDate && customEndDate) {
      filterDataByDateRange(CorrectionData, "custom");
      setShowCustomDatePicker(false);
    }
  };
