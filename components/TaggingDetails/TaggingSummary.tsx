...
import "react-datepicker/dist/react-datepicker.css";
  ...
  const [filteredData, setFilteredData] = useState<ITag[]>([]);
    ...
    const getData = async () => {
      try {
        ...
        const data = await fetchTaggingData();
          ...
          console.error(" tagg data:", data);
        settagData(data);
        filterDataByDateRange(data, dateRange);
      } catch (error) {
          console.error("Error fetching tagg data:", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  useEffect(() => {
    filterDataByDateRange(tagData, dateRange);
  }, [dateRange, customStartDate, customEndDate, tagData]);

  // Function to filter data by date range
  const filterDataByDateRange = (data: ITag[], range: string) => {
    if (!data || !data.length) {
  ...
  const filterByDateRange = (data: ITag[], start: Date, end: Date) => {
  ...
  const calculateSummary = () => {
    ...
    const totalTagLoss = totalIssuedWeight - totalReceivedWeight;
 ...
 const totalProcessingWeight = filteredData.reduce((sum, item) => {
    ...
    const TagLossPercentage = totalIssuedWeight
      ? ((totalTagLoss / totalIssuedWeight) * 100).toFixed(2)
      : "0";
    ...
    return [
      {
        iconClass: "fa-light fa-gem",
            title: "Tag Issued",
        value: totalTag.toString(),
        description: "Total tag jobs",
        percentageChange: "",
        isIncrease: true,
      },
        {
      ...
      },  
      {
        ...
        title: "Tag Loss",
        value: totalTagLoss.toFixed(2) + " g",
        description: TagLossPercentage + "% of issued",
        percentageChange: TagLossPercentage,
        isIncrease: false
      },
    ];
  };
  ...
  const handleApplyCustomRange = () => {
    if (customStartDate && customEndDate) {
      filterDataByDateRange(tagData, "custom");
      setShowCustomDatePicker(false);
    }
  };
